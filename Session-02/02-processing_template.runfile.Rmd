...
```{r}
cutoffs$nCount_RNA.min <- 5000
cutoffs$nFeature_RNA.min <- 250
...
```{r}
# boxplot(t(obj@assays$RNA@scale.data[c('CD4', 'CD3D', 'CD8A'),]))
# genes <- rowSums(obj@assays$RNA@counts[,1:15]) > 100
# genes <- genes[genes]
# genes <- genes %>% names()
# boxplot(obj@assays$RNA@scale.data[genes,1:15])
# boxplot(as.matrix(obj@assays$RNA@counts[genes,1:15]))
# as.data.frame(obj@assays$RNA@counts[genes,1:15]) %>%
#   reshape2::melt() %>%
# ggplot(aes(x=variable, y = value)) + 
#   geom_boxplot() + 
#   scale_y_log10() + 
#   labs(y='Expression count of genes', x = 'Cell') + 
#   theme_bw() +
#   theme(axis.text.x = element_blank())#, axis.ticks.x = element_blank())
# 
# as.data.frame(obj@assays$RNA@scale.data[genes,1:15]) %>%
#   reshape2::melt() %>%
# ggplot(aes(x=variable, y = value)) + 
#   geom_boxplot() + 
#   # scale_y_log10() + 
