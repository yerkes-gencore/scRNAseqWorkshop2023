all_markers_DT <- function(marker_table){
  marker_table %>%
    ...
    DT::datatable(
      ...
      caption='Markers within cluster relative to other cluster',
      filter='top',
...
pairwise_marker_DT <- function(marker_table) {
  marker_table %>%
  ...
  datatable(
    ...
    colnames = c('Gene',
                 ...
                 '% expressed in group 2'),
    caption='Markers between two groups',
    filter='top',
    autoHideNavigation = TRUE,
    extensions = c('Buttons', 'Scroller'),
    options = list(
