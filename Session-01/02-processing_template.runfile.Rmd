...
```{r}
ggplot(obj@meta.data, aes(x=nFeature_RNA, y=nCount_RNA, color=scDblFinder.class)) +
  geom_point(alpha=0.2) +
  scale_x_log10() +
  scale_y_log10() +
  theme_classic()
...
You could also manually specify your filtering thresholds. 
...
Now checking that cells fall within thresholds
...
```{r}
## Custom function, see 'R/functions_session-02.R' for more details
obj.cc_check <- filterLowGenes(obj, min.cells = 50, assay = 'RNA', calculate_only = FALSE)
```
...
obj.cc_check <- ScaleData(obj.cc_check,
  ...
  FindVariableFeatures(obj.cc_check) %>%
  RunPCA() %>%
...
```{r}
saveRDS(obj, here('Data/processed_obj.Rds'))
```
